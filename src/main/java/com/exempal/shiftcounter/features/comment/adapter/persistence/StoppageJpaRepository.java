...
public interface StoppageJpaRepository extends JpaRepository<StoppageEntity, Long> {
    @Override
    @EntityGraph(attributePaths = {"explanations", "explanations.author", "explanations.lastModifiedBy"})
    ...
    @EntityGraph(attributePaths = {"explanations", "explanations.author", "explanations.lastModifiedBy"})
    @Query("select distinct s from StoppageEntity s where s.shiftId in (select sh.id from ShiftEntity sh where sh.date = :date) and s.sensorKey = :sensorId and s.detectionType is not null and s.excluded = false")
    List<StoppageEntity> findSystemByShiftDateAndSensorId(@Param("date") LocalDate date,
                                                          @Param("sensorId") String sensorId);

    @EntityGraph(attributePaths = {"explanations", "explanations.author", "explanations.lastModifiedBy"})
    @Query("select distinct s from StoppageEntity s where s.shiftId in (select sh.id from ShiftEntity sh where sh.date between :from and :to) and s.sensorKey = :sensorId and s.detectionType is not null and s.excluded = false")
    ...
    @Query("""
            ...
            where s.shiftId = :shiftId and s.intervalIndex = :intervalIndex
              and s.state = :state and s.detectionType is not null and s.excluded = false
            """)
